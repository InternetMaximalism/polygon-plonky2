...
impl WhirConfig {
    /// Default WHIR configuration with rate = 1/64 (rate_bits = 6).
    ///
    /// This gives a code rate of 1/64, meaning each codeword is 64x the message length.
    /// Combined with 128-bit security target and appropriate query count.
    pub fn default_rate_64() -> Self {
        Self {
            rate_bits: 6,       // rate = 1/2^6 = 1/64
            num_queries: 28,    // Sufficient for 128-bit security at rate 1/64
            security_bits: 128,
            pow_bits: 20,
